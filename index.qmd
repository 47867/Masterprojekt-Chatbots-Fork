...
Als Plattform für den Fragebogen wurde SoSci Survey ausgewählt. Dabei handelt es sich um eine in der deutschsprachigen Forschung weit verbreitete und etablierte Plattform für die Durchführung von Onlinebefragungen, 
...
Da zwischen dem E-Mail-Verteiler und dem Whatsapp-Kanal nur wenige Studierende erreicht werden konnten, wurden weitere Teilnehmende im privaten Umfeld, über die Whatsapp Kanäle der Soziologie Studiengänge und über den Instagram-Kanal der Fachschftsinitiative Soziologie rekrutiert.
...
Die Fragebogendauer lag im Mittel bei `r overview_time |> select(mean) |> pull()`.
Die kürzeste Befragung dauerte dabei `r overview_time |> select(min) |> pull()`, während die längste `r overview_time |> select(max) |> pull()` andauerte.
...
Da die gespendeten Chatverläufe im Fragebogen entweder als JSON oder HTML hochgeladen wurden, wurden im ersten Schritt diese beiden Dateiformate separat aufbereitet, in ein Datenformat vereinheitlicht und ausschließlich auf User-Nachrichten gefiltert.
...
Die Nutzung eines LLMs hat hier gegenüber traditionellen Textklassifikationen mittels Machine Learning den großen Vorteil, dass sie deutlich leichter zu implementieren sind und keine aufwendigen Trainingsprozesse erfordern [@wang2023].
...
Das mit einer Accuracy von $0{,}87 - 0{,}90$ über alle Tasks und einem F1-Macro von $0{,}67 - 0{,}81$ am besten perfomende Modell war das zu dem Zeitpunkt aktuelle Frontier-Modell GPT-5.5.
...
Insbesondere bei der Klassifizierung von Inhalten könnte dies Dies könnte insbesondere die Klassifikation von Inhalten betreffen, da die hier analysierten Daten von drei CSS-Masterstudierenden stammen und dementsprechend eine hoheHomogenität der Inhalte zu erwarten ist.
...
Für das Erkennen des Aufgabentyps erziehlte das Modell eine Accuracy von 100%, bei kritischen Nachfragen lag die Accuracy bei 93.8% mit einem Macro-F1-Score von 88.1%.
Die schlechteste Performance zeigte das Modell mit einer Accuracy von 84.4% und einem Macro-F1-Score von 0.604 bei der Sentiment Klassifikation (@tbla-llm-metriken).
Dieser vergleichsweise niedrige Macro-F1-Score lässt sich insbesondere aud eine Tendenz von GPT-5.5 erklären, neutrale Chats als unfreundlich zu klassifizieren.
- dies geschah in vier Fällen (@figb-konfusssion).
...
Während die Selbstauskünfte der Teilnehmenden auf Personenebene vorlagen, wurden die gespendeten Chatlogs auf Chatebene klassifiziert und dann auf Personenebene aggregiert.
...
ine Ausnahme bildet die Skala zur sozialen Erwünschtheit, die zu einem Index zusammengefasst wurde.
...
Forschungsfrage 1 (siehe @sec-forschungsfragen) wird durch eine gemeinsame Betrachtung der Ergebnisse zur Informationsnutzung, zum kritischen Prüfen und zum Interaktionsstil aus beiden Datenquellen beantwortet.
Die drei Bereiche werden nicht zu einem gemeinsmaen Gesamtscore zusammengefasst, die ihre Diskrepanzmaße unterschiedlich skaliert sind (siehe @sec-operationalisierung).

Zur Beantwortung von Forschungsfrage 2 (siehe @sec-forschungsfragen) werden für jeden der fünf Aufgabenbereiche der Informationsnutzung der mittlere Surveyanteil, der mittlere Chatloganteil und die mittlere gerichtete Differenz ermittelt.
Zusätzlich werden die mittlere absolute Abweichung und der Anteil der Personen mit exakter Übereinstimmung bestimmt.
Die gerichtete Differenz zeigt, ob der Surveyanteil einer Nutzungsart im Durschnitt höher oder niedriger ist als der Chatloganteil.
