...
source(here::here("R_code", "02_data_analasys.R"))
...
## Nutzungsarten und aggregierte Verteilung: F3
...
Die Verteilung de r Nutzungsarten ist in @tbl-nutzungsarten dargestellt.
...
<!-- ??? Was hat F2 mit personenspezifischen unterschiede tub, allgemein ist mir etwas unklar was der unterschied zwischen F2 und F3 ist die lesen sich schon sehr ähnlich, also ich checke schon was der unteschied ist, aber finde die fragen etwas uneindeutig, vielleicht absolute und mittlere mehr unterscheiden in der Forschungsfrage--->

## Personenspezifische Unterschieder der Nutzungsarten: F2
...
<!-- ??? Also ich wieß  nicht wo das in den Forschugnsfragen steht außer bei F6 aber dafür machen wir ja dann die Clusterannaye--->

<!-- ??? wo kommen die p-werte im text her -> muss eine tablle in den appendix wo die raus zitiert werden können, aber weiße ehrlich gesagt auch nicht ob es braucht. Weil in der F2 steht ja nicht so richtig was dazu das man es nach gruppen unterscheidet--->

Explorativ wurde weiter untersucht, ob die Diskrepanz der Nutzungsarten mit ausgewählten personenbezogenen Merkmalen zusammenhing.
...
Bei den männlichen Teilnehmern betrug die durchschnittliche Total-Variation-Distanz `r gender_summary |> filter(gender_label == "Männlich") |> select(mean_discrepancy) |> mutate(mean_discrepancy = mean_discrepancy * 100) |> pull() |> formatC(digits = global_digits, format = "f", decimal.mark = ",")` % (Median=`r gender_summary |> filter(gender_label == "Männlich") |> select(median_discrepancy) |> pull() |> formatC(digits = global_digits, format = "f", decimal.mark = ",")`%), bei den weiblichen Teilnehmerinnen `r gender_summary |> filter(gender_label == "Weiblich") |> select(mean_discrepancy) |> mutate(mean_discrepancy = mean_discrepancy * 100) |> pull() |> formatC(digits = global_digits, format = "f", decimal.mark = ",")` % (Median=`r gender_summary |> filter(gender_label == "Weiblich") |> select(median_discrepancy) |> pull() |> formatC(digits = global_digits, format = "f", decimal.mark = ",")`%).
