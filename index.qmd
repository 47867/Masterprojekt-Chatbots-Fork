...
Diese Daten werden für jeden Chat jeder Person im nächsten Schritt mit OpenAIs GPT-5.5 nach Sentiment, kritischen Nachfragen und Aufgabenbereich klassifiziert.
...
LLMs zeigen bei Klassifikationsaufgaben, insbesondere der Klassifikation von Sentiment, recht gute Ergebnisse, die mit kostenaufwendigen "supervised learning" Methoden mithalten können [@wang2023; @anglin2025] und sogar mit den Annotationen von „Crowd-Workern" [@gilardi2023].
...
Das Modell, welches hier am besten performt hat, war das zu dem Zeitpunkt aktuelle Frontier-Modell GPT-5.5 mit einer Accuracy von $0{,}87 - 0{,}90$ über alle Tasks und einem F1-Makro von $0{,}67 - 0{,}81$. Die Ergebnisse erzielte es nach drei Runden der Promptverfeinerungen. Die detaillierten Evaluationen, mit allem Prompts und Modellspezifaktion ist hier zu finden [Online-Anhang](#sec-online-anhang) verfügbar.
...
Um das Problem der Übertragbarkeit zu adressieren, wurde eine Stichprobe von 30% der Survey-Datenspenden gezogen, um die von GPT-5.5 vergebenen Labels zu überprüfen.
Diiese wurden von zwei der Autor:innen anottiert, die Übereinstimmung der Coder beturg für Sentiment 90.6 % und für die beiden Anderen klassifikationen 100%.
...
Die Manhattan-Distanz und Euklidische-Distanz, benötigen zum Beispiel metrische oder binäre Daten, bzw. für die Euklidische-Distanz einen euklidischen Vektorraum.
...
Die Struktur des Algorithmus funktioniert wie folgt: Zu Beginn werden per Zufall drei Beobachtungen ausgewählt, das sind die  (dies wird durch einen Seed "404" replizierbar gehalten).
...
Schließlich werden die Beobachtungen, die Cluster-Zenteren (die Meddoid) durchrutiert. 
...
Dieser Prozess wird für $k = 2$ bis $k = 10$ wiederhol, also für bis zu einer Anzahl an 10 Clustern wiederholt.
Die Anzahl der Cluster, für die die maximale durchschnittliche Silhouette berechnet wird, wird verwendet. Nach der berechnung der Cluster, können diese noch auf inhaltliche Unterschiede überprüft werden.
...
Die Stabilität der Vier-Cluster-Lösung wurde mit drei Robustheitschecks geprüft, wie in der Präregestrierung angekündigt.
...
In der Präregestrierung wurd gesagt das die Anzahl der cluster für die Silluettenmethode auf sech beschränkt wird, das wurde auf zehn ausgeweitet
