...
Dieser erfolgte erst nach Abschluss der Befragung, um eine Beeinflussung der Sebsteinkünfte durch Einsicht in die eigenen Chatverläufe möglichst zu reduzieren. Jedoch kann eine Beeinflussung durch die Auseinandersetzung mit dem Fragebogen nicht vollends ausgeschlossen werden.
...
Weiterhin fiel auf, dass verschiedene technische Fehlerursachen in der Frage zu technischen Schwierigkeiten nicht eindeutig voneinander abgegrenzt waren. Um disjunkte Antwortoptionen zu ermöglichen, wurde die Frage daher in mehrere separate Fragen aufgeteilt.
...
Die kürzeste Befragung dauerte dabei `r formatC(overview_time |> select(min) |> pull(), format = "f", digits = global_digits, decimal.mark = ",")` Minuten, <!-- $$$ hoffe die war ungültig - das wäre noch gut zu ergänzen-->
<!-- @Hannah: die haben wir nicht ausgeschlossen -->
während die längste `r formatC(overview_time |> select(max) |> pull(), format = "f", digits = global_digits, decimal.mark = ",")` Minuten andauerte. Weitere Kennzahlen sind in @tbla-sample-comparison aufgeführt.
...
<!-- @Hannah: Ja, man könnte FF1 als übergeordnete FF verstehen und den Rest als FF1a - FF1d. -->
