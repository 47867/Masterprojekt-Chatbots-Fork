...
In den gespendeten Chatlogs konnten alle fünf untersuchten Nutzungsarten identifiziert werden. Technische und analytische Unterstützung trat mit 
...
Explorativ wurde untersucht, ob das Gesamtausmaß der Diskrepanz mit ausgewählten personenbezogenen Merkmalen zusammenhing. Für die AI-Erfahrung zeigte sich ein schwacher bis moderater positiver Zusammenhang mit der Diskrepanz ($ρ=$ `r association_summary |> filter(variable == "AI-Erfahrung") |> select(rho) |> pull() |> round(digits = global_digits)`, $p=$ `r association_summary |> filter(variable == "AI-Erfahrung") |> select(p_value) |> pull() |> round(digits = global_digits)`). Personen mit größerer AI-Erfahrung wiesen somit deskriptiv eher größere Abweichungen zwischen Selbstauskunft und beobachteter Nutzung auf. Der Zusammenhang war jedoch statistisch unsicher. 
...
Insgesamt liefern die Ergebnisse keine belastbaren Hinweise darauf, dass Alter, AI-Erfahrung oder soziale Erwünschtheit systematisch mit dem Ausmaß der Diskrepanz zusammenhängen. Der Geschlechterunterschied stellt ein auffälliges deskriptives Muster dar, das aufgrund der kleinen Stichprobe und der explorativen Analyse jedoch nicht verallgemeinert werden kann.
