...
Damit lag der selbstberichtete Anteil um `r formatC(aggregate_comparison[[1, "Differenz"]] * 100, format = "f", digits = global_digits, decimal.mark = ",")` Prozentpunkte unter dem beobachteten Anteil.
...
Bei *`r aggregate_comparison[[2, "task"]]`* war der Surveyanteil um `r formatC(aggregate_comparison[[2, "Differenz"]] * 100, format = "f", digits = global_digits, decimal.mark = ",")` Prozentpunkte niedriger.
Die aggregierten Unterschiede waren somit – abgesehen von technischer Unterstützung und Schreiben – gering.
Die Verteilung der Nutzungsarten ist in @tbl-nutzungsarten dargestellt.
...
Knapp die Hälfte der Teilnehmenden gab an, ChatGPT-Antworten im Chat kritisch zu prüfen (`r critical_cross_table |> filter(self_report == "Nein", observed == "Nein") |> select(percent) |> pull() |> formatC(digits = global_digits, decimal.mark = ",", format = "f")` %).
...
In Hinblick auf F4 (siehe @sec-forschungsfragen) deuten die Ergebnisse der kritischen Prüfung darauf hin, dass die Selbsteinschätzung der Teilnehmenden tendenziell korrekt ausfiel, bei Abweichungen aber insgesamt optimistischer angegeben wurde als das tatsächliche Verhalten in den Chatlogs zeigte.
...
Explorativ wurde weiter untersucht, ob das Gesamtausmaß der Diskrepanz mit ausgewählten personenbezogenen Merkmalen zusammenhing.
Für die Erfahrung mit generativer KI zeigte sich ein schwacher bis moderater positiver Zusammenhang mit der Diskrepanz ($ρ=$ `r association_summary |> filter(variable == "AI-Erfahrung") |> select(rho) |> pull() |> round(digits = global_digits)`, $p=$ `r association_summary |> filter(variable == "AI-Erfahrung") |> select(p_value) |> pull() |> round(digits = global_digits)`).
Personen mit größerer Erfahrung mit KI wiesen somit deskriptiv eher größere Abweichungen zwischen Selbstauskunft und beobachteter Nutzung auf.
Der Zusammenhang war jedoch statistisch unsicher.
...
Deutlichere deskriptive Unterschiede zeigten sich nach Geschlecht.
Bei den männlichen Teilnehmern betrug die durchschnittliche Total-Variation-Distanz `r gender_summary |> filter(gender_label == "Männlich") |> select(mean_discrepancy) |> pull() |> formatC(digits = global_digits, format = "f", decimal.mark = ",")` % (Median=`r gender_summary |> filter(gender_label == "Männlich") |> select(median_discrepancy) |> pull() |> formatC(digits = global_digits, format = "f", decimal.mark = ",")`%), bei den weiblichen Teilnehmerinnen `r gender_summary |> filter(gender_label == "Weiblich") |> select(mean_discrepancy) |> pull() |> formatC(digits = global_digits, format = "f", decimal.mark = ",")` % (Median=`r gender_summary |> filter(gender_label == "Weiblich") |> select(median_discrepancy) |> pull() |> formatC(digits = global_digits, format = "f", decimal.mark = ",")`%).
Die durchschnittliche Diskrepanz lag damit bei den Männern um `r gender_summary |> filter(gender_label == "Männlich") |> select(sd_discrepancy) |> pull() |> formatC(digits = global_digits, format = "f", decimal.mark = ",")` Prozentpunkte höher.
Der Unterschied erreichte im Wilcoxon-Rangsummentest jedoch nicht das konventionelle Signifikanzniveau von 5 % ($W=$ `r gender_test$statistic`, $p=$ `r formatC(gender_test$p.value, digits = global_digits, format = "f", decimal.mark = ",")`).
...
Der Geschlechterunterschied stellt ein auffälliges deskriptives Muster dar, das aufgrund der kleinen Stichprobe und der explorativen Analyse jedoch nicht verallgemeinert werden kann.
