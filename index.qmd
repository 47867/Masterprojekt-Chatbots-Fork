...
Dabei werden sowohl die Konzeption und Durchführung der Fragebogenerhebung als auch die Erhebnung und Aufbereitung der Chatlogs erläutert.
...
Als Plattform für den Fragebogen wurde SoSci Survey, eine, vor allem in der deutschsprachigen Forschung, weit verbreitete, etablierte Plattform für die Durschführung von Onlinebefragungen, ausgewählt.
Dort können die Daten auf Servern in Deutschland gespeichert werden, wodurch die Einhaltung der Datenschutzgrundverordnung (DSGVO) gewährleistet ist.
...
Wer angab, nicht an einer Hochschule immatrikuliert zu sein, ChatGPT generell nicht, oder nicht im Studienkontext zu nutzen, wurde von dem weiteren Aufüllen des Fragebogens ausgeschlossen.
...
Da zwischen dem E-Mail-Verteiler und dem Whatsapp-Kanal nur insgesamt 27 Studierende erreicht werden konnten, wurden weitere Teilnehmende im privaten Umfeld, über die Whatsapp Kanäle der Soziologie Studiengänge und über den Instagram-Kanal der Fachschftsinitiative Soziologie rekrutiert.
...
Die kürzeste Befragung dauerte dabei `r formatC(overview_time |> select(min) |> pull(), format = "f", digits = global_digits, decimal.mark = ",")` Minuten, während die längste `r formatC(overview_time |> select(max) |> pull(), format = "f", digits = global_digits, decimal.mark = ",")` Minuten andauerte.
...
Während die Selbstauskünfte der Teilnehmenden auf Personenebene vorlagen, wurden die gespendeten Chatlogs auf Chatebene klassifiziert und mussten vor der weiteren Analyse ebenfalls auf Personenebene (wide-format) transformiert und dann auf Personenebene aggregiert werden.
...
Auf diese Weise wird im Anschluss eine direkte Vergleichbarkeit zwischen Selbstauskunft und Verhaltensdaten ermöglicht.
